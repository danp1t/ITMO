...
select Н_ЛЮДИ.ИД, Н_ЛЮДИ.ФАМИЛИЯ, Н_ЛЮДИ.ИМЯ, Н_ЛЮДИ.ОТЧЕСТВО from Н_ЛЮДИ where Н_ЛЮДИ.ИД not in
    (select Н_ОБУЧЕНИЯ.ЧЛВК_ИД from Н_ОБУЧЕНИЯ where Н_ОБУЧЕНИЯ.ЧЛВК_ИД in 
            (select Н_УЧЕНИКИ.ЧЛВК_ИД from Н_УЧЕНИКИ) UNION (select Н_ОБУЧЕНИЯ.ЧЛВК_ИД from Н_ОБУЧЕНИЯ where Н_ОБУЧЕНИЯ.ЧЛВК_ИД in (select Н_УЧЕНИКИ.ЧЛВК_ИД from Н_УЧЕНИКИ where Н_УЧЕНИКИ.ПРИЗНАК = 'диплом')));



select Н_УЧЕНИКИ.ИД from Н_УЧЕНИКИ where Н_УЧЕНИКИ.ПЛАН_ИД in 
    (select Н_ПЛАНЫ.ИД from Н_ПЛАНЫ where Н_ПЛАНЫ.ФО_ИД = (select Н_ФОРМЫ_ОБУЧЕНИЯ.ИД from Н_ФОРМЫ_ОБУЧЕНИЯ where Н_ФОРМЫ_ОБУЧЕНИЯ.ИМЯ_В_ИМИН_ПАДЕЖЕ = 'вечерняя') and Н_УЧЕНИКИ.ЧЛВК_ИД in 
                (select Н_ЛЮДИ.ИД from Н_ЛЮДИ where date_part('year', age(Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) < 20));

Select Н_ЛЮДИ.ИД, Н_ЛЮДИ.ИМЯ, date_part('year', age(Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) from Н_ЛЮДИ INNER JOIN Н_УЧЕНИКИ on (Н_ЛЮДИ.ИД = Н_УЧЕНИКИ.ЧЛВК_ИД) where Н_УЧЕНИКИ.ПЛАН_ИД in (select Н_ПЛАНЫ.ИД from Н_ПЛАНЫ where Н_ПЛАНЫ.ФО_ИД = (select Н_ФОРМЫ_ОБУЧЕНИЯ.ИД from Н_ФОРМЫ_ОБУЧЕНИЯ where Н_ФОРМЫ_ОБУЧЕНИЯ.ИМЯ_В_ИМИН_ПАДЕЖЕ = 'вечерняя') and Н_УЧЕНИКИ.ЧЛВК_ИД in 
                (select Н_ЛЮДИ.ИД from Н_ЛЮДИ where date_part('year', age(Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) < 20));


select Н_ЛЮДИ.ИД, Н_ЛЮДИ.ИМЯ, date_part('year', age(Н_ЛЮДИ.ДАТА_РОЖДЕНИЯ)) from Н_ЛЮДИ where Н_ЛЮДИ.ИД in ( 
